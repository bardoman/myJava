...
public class SortableTableModel extends AbstractTableModel {
    ...
    public int modelIndex(int viewIndex) {
        Row oneRow = getViewToModel()[viewIndex];
        return oneRow.modelIndex;// broke this into 2 lines  because I'm getting a null here.
    }

    public Vector getSorteDataVector() {
        Vector tableDataVector =new Vector();
        for (int i = 0;i<getRowCount();i++) {
            Vector temp = new Vector();
            for (int j=0;j<getColumnCount();j++) {
                temp.add(getValueAt(i,j));
            }
            tableDataVector.add(temp);
        }
