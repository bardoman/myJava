...
public class Torus {
    ...
    String bigStr="";
    char  bigCharAray[]=null;

    int locs[]=null;

    int messageCount=0;

    public Torus(String fileName, boolean fillRand)throws Exception
    {
        ...
        while (rd.ready()) {
            ...
            if (tmp.length()!=0) {
                totalMessageLength+=tmp.length();
                tmpVect.add(cnt+removeSpaces(tmp)+"*");
            }
            cnt++;
        }


        Collections.shuffle(tmpVect);

        messageCount=tmpVect.size();
        msgs=new String[messageCount];

        int spacerSize = (totalMessageLength / messageCount)*2;

        char charSpacer []= new char [spacerSize];

        Arrays.fill(charSpacer,' ');

        String strSpacer = new String(charSpacer);

        for (int n=0;n!=messageCount;n++) {
            bigStr+=(String)  tmpVect.get(n)+ strSpacer;
        }

        //  bigStr.getChars(0,bigStr.length(),bigCharAray,0);


        // torus = new Character[totalMessageLength*2);

        //addMessages();

        locs = getLocations(bigStr,messageCount);
    ...
    void printPage(int pageNum,char page[][])
    {
        ...
        for (int row=0;row!=pageRows;row++) {
            ...
            System.out.println(rowHdr+":"+str+":"+rowHdr);
        }
...
*/
    ...
    { int loc=-1;
        ...
        for (int i=0;i!=bigStr.length();i++) {
            char ch1 = bigStr.charAt(i);
            if (ch1==' ') {
                total += new Character(legalChars[rand.nextInt(legalChars.length)]);
            } else {
                total+=ch1;
            }
        } 
    ...
    int [] getLocations(String str, int messageCount)
    {   
        int locs[]=new int[messageCount];    

        for (int i=0;i<=messageCount;i++) {
            String tmpStr=str;
            String locNumStr = String.valueOf(i);
            int loc = tmpStr.indexOf(locNumStr);
            int totalLoc=0;

            while (true) {

                int nextLoc=loc+locNumStr.length();
                if (!Character.isDigit(tmpStr.charAt(nextLoc))) {
                    if (totalLoc!=0) {
                        loc=totalLoc+loc;
                    }
                    break;
                } else {
                    tmpStr=tmpStr.substring(nextLoc+1);
                    loc = tmpStr.indexOf(locNumStr);
                    totalLoc+=loc;
                }
            }

            if (loc != -1) {
                System.out.println(str.substring(loc, loc+20));
                locs[i]=loc;
            }
        }
        return locs;
    ...
    public static void main(String args[])
    {
        if (args.length <1) {
            ...
            if (args.length==2) {
                if (args[1].toUpperCase().equals("NO")) {
                    randFill = false;
                }
            }

            try {
                new Torus(args[0],randFill);
            } catch (Exception e) {
