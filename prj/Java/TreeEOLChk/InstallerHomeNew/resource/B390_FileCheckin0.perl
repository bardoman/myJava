...
# get parameters from CMVC except parmfile
   if ($debug) {
      logit("-------------------------------------------------------------\n", "no");
      logit("Build390 user exit - FileCheckin 0 - args = @ARGV\n");
   }
   ($pathName, $tempFile, $release, $tempForce, $tempRemarks, $tempFlag, $defect) = @ARGV;
...
#connect to Build/390 service server & send exit info
   ...
   print $remote "FileCheckIn pathname=$pathName release=$release defect=$defect\n\n" . $BLANK;
