#!/usr/bin/perl
# perl script
# B390_TestAccept1 Pre Process user exit
...
# get parameters from CMVC except parmfile
   if ($debug) {
      logit("-------------------------------------------------------------\n", "no");
      logit("Build390 user exit - Test accept 1 - args = @ARGV\n");
   }
...
#connect to Build/390 service server & send exit info
   ...
   print $remote "TestAccept defect=$defect release=$release tester=$owner environment=$envirName \n\n" . $BLANK;
