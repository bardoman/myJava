#!/usr/bin/perl
# perl script
# B390_TestReject2 Post Process user exit
...
# get parameters from CMVC except parmfile
   if ($debug) {
      logit("-------------------------------------------------------------\n", "no");
      logit("Build390 user exit - Test reject 1 - args = @ARGV\n");
   }
   ($defect, $owner, $release, $environment) = @ARGV;
...
#connect to Build/390 service server & send exit info
   ...
   print $remote "TestReject defect=$defect release=$release  tester=$owner  environment=$environment \n\n" . $BLANK;
