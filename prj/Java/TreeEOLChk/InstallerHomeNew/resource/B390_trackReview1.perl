...
# 09/20/00   change to parse the return code string that is fed in the format: RC=0:WARNING...
#            (shouldnt impact other perl progs 'cause the MBClient program that sends the string  adds it only if the warningstring is not null)
...
if ($debug) {
   logit("-------------------------------------------------------------\n", "no");
   logit("Build390 user exit - track review 1 - args = @ARGV\n");
}
...
#connect to Build/390 service server & send exit info
   ...
   print $remote "TrackReview defect=$defect release=$release \n\n" . $BLANK;
# this pipes any output from the server  back to the client.
   ...
   if ($returnString=~/error/i) {
      ...
      print "Promotion to review successful.\n";
   }
